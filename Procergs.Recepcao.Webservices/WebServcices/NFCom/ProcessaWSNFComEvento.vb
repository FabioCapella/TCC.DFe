...
Namespace NFComRecepcao
    ...
    Public Class ProcessaWSNFComEvento
        Inherits ProcessaWSNFComDFe

        Private Homologado As Boolean = False

        Public Sub New()
            WSNFCom.CodWS = CodWS.Evento
            WSNFCom.CodTipoDocXML = TipoDocXMLDTO.TpDocXML.NFCOMEvento
        End Sub
        ...
        Private Function Homologar() As XmlDocument
            ...
            Dim protocolo As New TNFComProtocoloInfEvento
            Dim xmlResposta As New XmlDocument
            Try

                Dim retAut As RetornoValidacaoDFe = Validador.ValidarEvento(WSNFCom.XMLDados, DFeTiposBasicos.TpDFe.NFCOM, New ValidadorConfig() With {.IgnoreSchema = True})
                protocolo = DFeProtocolo.ObterProtocoloNFComEvento(retAut.CodSituacao, retAut.Descricao, Validador.Rejeitado, Validador.EventoValidar, Validador.Reserva)
                ...
                If Not Validador.Rejeitado Then
                    ...
                    Dim sRet As String = NFComRecepcaoDAO.InserirEvento(Validador.EventoValidar, protocolo, WSNFCom)
                    If sRet <> "0" Then
                        Validador.Rejeitado = True
                        If sRet.IndexOf("duplicate") >= 0 Then
                            Dim eventoDup As EventoDTO = NFComEventoDAO.ObtemPorChaveAcesso(Validador.EventoValidar.ChaveAcesso, Validador.EventoValidar.TipoEvento, Validador.EventoValidar.NroSeqEvento, Validador.EventoValidar.Orgao)
                            If eventoDup IsNot Nothing Then
                                ...
                                protocolo = DFeProtocolo.ObterProtocoloNFComEvento(631, SituacaoCache.Instance(NFComEvento.SiglaSistema).ObterSituacao(631).Descricao, True, Validador.EventoValidar)
                            End If
                            Return MontarResposta(protocolo.cStat, protocolo)
                        End If
                        DFeLogDAO.LogarEvento("ProcessaWSNFComEvento", "Retorno 999. Erro ao tentar Registrar Evento NFCom: " & sRet, DFeLogDAO.TpLog.Erro, True, WSNFCom.Operario.ToString, WSNFCom.CodInscrMFTransm, WSNFCom.RemoteAddr, False)
                        Status = 999
                        Return MontarResposta(Status)
                    End If
            ...
            Catch ex As Exception
                DFeLogDAO.LogarEvento("ProcessaWSNFComEvento", "ERRO. XML: " & DadosMsg & "Erro:" & ex.Source & "-" & ex.Message & "-" & ex.StackTrace, DFeLogDAO.TpLog.Erro, True, WSNFCom.Operario.ToString, WSNFCom.CodInscrMFTransm, WSNFCom.RemoteAddr, False)
                Throw ex
            End Try

        End Function

        Public Function MontarResposta(status As String, Optional protocolo As TNFComProtocoloInfEvento = Nothing) As XmlDocument
            Dim oXmlDoc As New XmlDocument

            Try

                oXmlDoc.LoadXml("<retEventoNFCom/>")
                ...
                ocOrgao.InnerText = WSNFCom.CodUFOrigem
                ...
                If protocolo IsNot Nothing AndAlso protocolo.nProtSpecified Then
                    Dim ochNFCom As XmlElement = infEvento.CreateElement("chNFCom")
                    ...
                    ochNFCom.InnerText = protocolo.chNFCom
                    ...
                    infEvento.DocumentElement.AppendChild(ochNFCom)
                ...
                Dim oNodeList As XmlNodeList = oXmlDoc.GetElementsByTagName("retEventoNFCom")
