...
Namespace CTeRecepcao
    ...
    Public Class ProcessaWSCTeOS
        Inherits ProcessaWSCTeDFe

        Private Homologado As Boolean = False
        Private m_iCod_int_cert_ASSINAT As Integer = 0
        Private m_iCod_int_HLCR_ASSINAT As Integer = 0
        Private m_sExtCNPJCert As String

        Public Sub New()
            WSCTe.CodWS = CTeRecepcao.CodWS.RecepcaoCTeOS
            WSCTe.CodTipoDocXML = TipoDocXMLDTO.TpDocXML.CTeOS
        End Sub
        ...
        Private Function Homologar() As XmlDocument
            ...
            Try
                ...
                Try
                    ...
                    Dim retAut As RetornoValidacaoDFe = Validador.ValidarDFe(WSCTe.XMLDados, DFeTiposBasicos.TpDFe.CTeOS, New ValidadorConfig() With {.IgnoreSchema = True})
                    ...
                    If Not Validador.Rejeitado Then
                        ...
                        Else
                            ...
                            Else
                                ...
                                DFeLogDAO.LogarEvento("ProcessaWSCTeOS", "ATENÇÃO: Falha ao gravar CTeOS. " & Validador.DFeValidar.ChaveAcesso & " Alterado o retorno para: 999 " & protocolo.xMotivo & ". Erro interno : " & sRet, DFeLogDAO.TpLog.Erro, True, WSCTe.Operario.ToString, WSCTe.CodInscrMFTransm, WSCTe.RemoteAddr, False)
                            End If
                ...
                Catch exCTe As Exception
                    ' Erro não catalogado
                    DFeLogDAO.LogarEvento("ProcessaWSCTeOS", "ATENÇÃO: Falha ao gravar Remessa. " & Validador.DFeValidar.ChaveAcesso & " Erro interno : " & exCTe.Message, DFeLogDAO.TpLog.Erro, True, WSCTe.Operario.ToString, WSCTe.CodInscrMFTransm, WSCTe.RemoteAddr, False)
                    protocolo = DFeProtocolo.ObterProtocoloCTe(999, SituacaoCache.Instance(CTe.SiglaSistema).ObterSituacao(999).Descricao, True, Validador.DFeValidar)
                    Validador.Rejeitado = True
                End Try
                ...
                If Validador.Rejeitado Then
                    Try
                        ...
                        If Not IsNumeric(sRet) Then
                            DFeLogDAO.LogarEvento("ProcessaWSCTeOS", "ATENÇÃO: Falha ao inserir rejeição de CTeOS " & Validador.DFeValidar.ChaveAcesso & " . Erro interno : " & sRet, DFeLogDAO.TpLog.Erro, True, WSCTe.Operario.ToString, WSCTe.CodInscrMFTransm, WSCTe.RemoteAddr, False)
                            Throw New Exception(sRet)
                        End If
                    Catch ex As Exception
                        DFeLogDAO.LogarEvento("ProcessaWSCTeOS", "ATENÇÃO: Falha ao inserir rejeição de CTeOS " & Validador.DFeValidar.ChaveAcesso & " . Erro interno : " & ex.Message, DFeLogDAO.TpLog.Erro, True, WSCTe.Operario.ToString, WSCTe.CodInscrMFTransm, WSCTe.RemoteAddr, False)
                        Throw ex
                    End Try
                End If
        ...
        Public Function MontarResposta(status As String, Optional protocolo As TCTeProtocoloInfProt = Nothing) As XmlDocument
            ...
            Try
                ...
                If protocolo IsNot Nothing AndAlso status = 100 Then
                    Dim oNodeList As XmlNodeList = oXmlDoc.GetElementsByTagName("retCTeOS")
