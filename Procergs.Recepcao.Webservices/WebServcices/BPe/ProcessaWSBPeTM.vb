...
Namespace BPeRecepcao
    ...
    Public Class ProcessaWSBPeTM
        Inherits ProcessaWSBPeDFe

        Private Homologado As Boolean = False

        Public Sub New()
            WSBPe.CodWS = BPeRecepcao.CodWS.BPeTM
            WSBPe.CodTipoDocXML = TipoDocXMLDTO.TpDocXML.BPeTM
        End Sub

        Public Function AutorizarBPeTM() As XmlDocument
        ...
        Private Function Homologar() As XmlDocument
            ...
            Try
                    ...
                    If Not Validador.Rejeitado Then
                        ...
                        Else
                            ...
                            Else
                                ...
                                DFeLogDAO.LogarEvento("ProcessaWSBPeTM", "ATENÇÃO: Falha ao gravar BPe. " & Validador.DFeValidar.ChaveAcesso & " Alterado o retorno para: 999 " & protocolo.xMotivo & ". Erro interno : " & sRet, DFeLogDAO.TpLog.Erro, True, WSBPe.Operario.ToString, WSBPe.CodInscrMFTransm, WSBPe.RemoteAddr, False)
                            End If
                ...
                Catch exBPe As Exception
                    ' Erro não catalogado
                    DFeLogDAO.LogarEvento("ProcessaWSBPeTM", "ATENÇÃO: Falha ao gravar Remessa. " & Validador.DFeValidar.ChaveAcesso & " Erro interno : " & exBPe.Message, DFeLogDAO.TpLog.Erro, True, WSBPe.Operario.ToString, WSBPe.CodInscrMFTransm, WSBPe.RemoteAddr, False)
                    protocolo = DFeProtocolo.ObterProtocoloBPe(999, SituacaoCache.Instance(BPe.SiglaSistema).ObterSituacao(999).Descricao, True, Validador.DFeValidar)
                    Validador.Rejeitado = True
                End Try
                ...
                If Validador.Rejeitado Then
                    Try
                        ...
                        If Not IsNumeric(sRet) Then
                            DFeLogDAO.LogarEvento("ProcessaWSBPeTM", "ATENÇÃO: Falha ao inserir rejeição de BPe " & Validador.DFeValidar.ChaveAcesso & " . Erro interno : " & sRet, DFeLogDAO.TpLog.Erro, True, WSBPe.Operario.ToString, WSBPe.CodInscrMFTransm, WSBPe.RemoteAddr, False)
                            Throw New Exception(sRet)
                        End If
                    Catch ex As Exception
                        DFeLogDAO.LogarEvento("ProcessaWSBPeTM", "ATENÇÃO: Falha ao inserir rejeição de BPe " & Validador.DFeValidar.ChaveAcesso & " . Erro interno : " & ex.Message, DFeLogDAO.TpLog.Erro, True, WSBPe.Operario.ToString, WSBPe.CodInscrMFTransm, WSBPe.RemoteAddr, False)
                        Throw ex
                    End Try
                End If
